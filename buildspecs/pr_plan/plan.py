...
def main() -> None:
    ...
    for account in json.loads(os.environ["ACCOUNT_DIM"]):
        ...
        diff_paths = []
        for diff in repo.heads.master.commit.diff(
            os.environ["CODEBUILD_RESOLVED_SOURCE_VERSION"],
            paths=[f'{account["path"]}/**.hcl', f'{account["path"]}/**.tf'],
        ):
        ...
        if len(diff_paths) > 0:
            ...
            log.info(
                "No New/Modified Terraform configurations within account -- skipping Terraform plan"
            )
