...
@pytest.mark.parametrize(
    "repo_changes,plan_side_effect,expected_plan_dirs,expected_failure",
    [
        pytest.param(
            {
...
def test_main(
    ...
    expected_cmds = [
        f'terragrunt plan --terragrunt-working-dir {dir} --terragrunt-iam-role {account_dim[0]["plan_role_arn"]}'.split(
            " "
        )
        for dir in expected_plan_dirs
    ]
