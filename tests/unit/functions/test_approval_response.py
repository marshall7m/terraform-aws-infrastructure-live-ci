...
@pytest.mark.parametrize('event,record,status,expected_status_code,expected_send_token', [
    pytest.param(
        {
            'body': {
                'action': 'approve',
...
def test_lambda_handler(mock_sf, conn, cur, event, record, status, expected_status_code, expected_send_token):
    ...
    if record != {}:
        ...
        cur.execute(sql.SQL('SELECT approval_voters, rejection_voters FROM executions WHERE execution_id = {}').format(sql.Literal(event['query']['ex'])))
    ...
    if expected_send_token:
        mock_sf.send_task_success.assert_called_with(taskToken=event['query']['taskToken'], output=json.dumps(event['body']['action']))