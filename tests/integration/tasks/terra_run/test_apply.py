...
def test_successful_execution(mut_output, push_changes):
    ...
    run_task(
        ...
        compose_files=[
            os.path.join(
                os.path.dirname(__file__), "docker-compose.ecs-local.apply.yml"
            )
        ],
        task_env_vars={
            "STATUS_CHECK_NAME": status_check_name,
            "CFG_PATH": cfg_path,
            "ROLE_ARN": mut_output["ecs_apply_role_arn"],
            # "IS_ROLLBACK": "false",
            # "NEW_PROVIDERS": ,
            "EXECUTION_ID": "run-123",
            "TG_COMMAND": f"terragrunt apply --terragrunt-working-dir {cfg_path} --terragrunt-iam-role {mut_output['apply_role_arn']} -auto-approve",
            "STATE_NAME": "Apply",
