...
| <a name="module_terra_run_role"></a> [terra\_run\_role](#module\_terra\_run\_role) | github.com/marshall7m/terraform-aws-iam//modules/iam-role | v0.1.0 |
...
| <a name="input_ecs_tasks_common_env_vars"></a> [ecs\_tasks\_common\_env\_vars](#input\_ecs\_tasks\_common\_env\_vars) | Common env vars defined within all Codebuild projects. Useful for setting Terragrunt specific env vars required to run Terragrunt commands. | <pre>list(object({<br>    name  = string<br>    value = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_enable_merge_lock"></a> [enable\_merge\_lock](#input\_enable\_merge\_lock) | Determines if merge lock will be enabled | `bool` | `true` | no |
| <a name="input_enable_pr_plan"></a> [enable\_pr\_plan](#input\_enable\_pr\_plan) | Determines if PR plans will be created | `bool` | `true` | no |
